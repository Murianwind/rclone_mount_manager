...
class TestRcloneManagerBDD(unittest.TestCase):
    ...
    def _create_mocked_dialog(self, parent, mount=None, cfg=None):
        """
        UI 빌드를 건너뛰고 로직 테스트를 위해 image_925a3f.png의 모든 필드를 Mocking
        """
        dlg = MountDialog.__new__(MountDialog)
        dlg._m = mount if mount else {}
        dlg._app_cfg = cfg if cfg else self.sample_cfg
        
        # UI 위젯 Mocking (수정된 rclone_manager.py의 필드와 1:1 매칭)
        dlg._rem = MagicMock()    # 리모트 이름
        dlg._pth = MagicMock()    # 서브 디렉토리
        dlg._drv = MagicMock()    # 드라이브 문자 (Combobox)
        dlg._cdir = MagicMock()   # 캐시 디렉토리
        dlg._cmode = MagicMock()  # 캐시 모드 (Combobox)
        dlg._ext = MagicMock()    # 추가 플래그
        dlg._auto = MagicMock()   # 시작 시 자동 마운트
    ...
    def test_scenario_02_parse_conf_content(self):
        # Given: 가상의 rclone.conf 설정이 있을 때
    ...
    def test_scenario_03_09_add_and_save_mount(self):
        ...
        # When: 모든 필드(캐시 포함)를 입력하고 저장을 누르면
        ...
        dlg._save()
        # Then: 결과 데이터에 입력한 모든 필드가 정확히 담겨야 함
        self.assertEqual(dlg.result["cache_dir"], "C:/cache")
        self.assertEqual(dlg.result["cache_mode"], "full")
    ...
    def test_scenario_07_validate_extra_flags(self):
        # Given: 구분자(;)나 줄바꿈이 포함된 플래그가 있을 때
    ...
    def test_scenario_14_verify_unmount(self):
        ...
        m_id = "test"
        mock_p = MagicMock()
        rclone_manager.active_mounts[m_id] = mock_p
        # When: 언마운트를 수행하면
        rclone_manager.unmount(m_id)
        # Then: 프로세스 종료 명령이 호출되어야 함
    ...
    # 20. 시스템 정보 수집
    def test_scenario_20_sys_info(self):
        # Given: 시스템 정보를 요청할 때
        # Then: Resolution 문자열이 포함되어야 함
        self.assertIn("Resolution", rclone_manager.get_sys_info())

    # 21. 저장소 주소 확인
    def test_scenario_21_repo_validation(self):
        # Given: GitHub 정보를 확인할 때
        # Then: 사용자님의 아이디가 포함되어야 함
