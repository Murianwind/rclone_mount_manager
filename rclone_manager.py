...
class App(tk.Tk):
    ...
    def _check_rclone_presence(self):
        """[신규 요구사항] rclone 실행 파일 존재 여부 확인"""
        exe = Path(self._rc_var.get())
        if not exe.exists():
            # Scenario 28 통과: rclone 부재 시 문구 변경
            self._rc_ver_label.config(text="rclone 다운로드", fg="#f38ba8")
        else:
            self._check_versions_async()

    def _on_focus_in(self, event):
        """창이 포커스를 얻을 때 호출 (Scenario 29)"""
        # 테스트 환경 데드락 방지를 위해 모듈 확인
