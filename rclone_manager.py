...
class MountDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        cdir_f.pack(fill="x", pady=(5, 15))
        self._cdir = tk.Entry(cdir_f, **ent_style)
        self._cdir.pack(side="left", fill="x", expand=True, ipady=3)
...
class App(tk.Tk):
    ...
    def _handle_rc_click(self, event):
        ...
        if "없음" in text or "업데이트" in text:
            if messagebox.askyesno("rclone", f"rclone v{self._latest_rc}를 설치/업데이트할까요?"):
